...
class ToshibaController final : public climate::Climate, public Component {
    ...
    void handle_register_fan_mode(ToshibaFanMode value) {
        switch (value) {
            ...
            case ToshibaFanMode::FAN_LOW_MEDIUM:
                ...
                this->set_custom_fan_mode_(CUSTOM_FAN_MODE_LOW_MEDIUM.c_str());
                break;
            ...
            case ToshibaFanMode::FAN_MEDIUM_HIGH:
                ...
                this->set_custom_fan_mode_(CUSTOM_FAN_MODE_MEDIUM_HIGH.c_str());
                break;
    ...
    void configure_capabilities() {
        ...
        supported_traits_.set_supported_custom_fan_modes({"Low Medium", "Medium High"});
...
public:
    ...
    // Switch setters for codegen
    void set_internal_thermistor_switch(CustomSwitch* sw) {
        switch_internal_thermistor_ = sw;
        sw->set_controller(this);
        sw->set_type(SWITCH_INTERNAL_THERMISTOR);
    }
    void set_ionizer_switch(CustomSwitch* sw) {
        switch_ionizer_ = sw;
        sw->set_controller(this);
    ...
    void control_handle_custom_fan_mode(const climate::ClimateCall& call) {
        ...
        this->set_custom_fan_mode_(custom_mode);

        if (custom_mode == CUSTOM_FAN_MODE_LOW_MEDIUM) {
            this->request_write_register_(ToshibaCommand::FAN_MODE, ToshibaFanMode::FAN_LOW_MEDIUM);
            return;
        }
        if (custom_mode == CUSTOM_FAN_MODE_MEDIUM_HIGH) {
            this->request_write_register_(ToshibaCommand::FAN_MODE, ToshibaFanMode::FAN_MEDIUM_HIGH);
            return;
        }
        ESP_LOGE(TAG, "received unknown custom fan mode: %s", std::string(custom_mode).c_str());
    }
